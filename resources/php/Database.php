<?php
	...
	class Database {
		 ...
		 */
		public function __construct($db = null,$host = null, $username = null, $password = null){
			global $db_NAME, $db_HOST, $db_USER, $db_PASS;
			if (!isset($db)) $db = $db_NAME;
			if (!isset($host)) $host = $db_HOST;
			if (!isset($username)) $username = $db_USER;
			if (!isset($password)) $password = $db_PASS;
		 ...
		 * @return array Contains the returned rows from the select query.
		 */
		public function numRows($tableName, $numRows = null, $columns = '*') 
		{
			$res = $this->get ($tableName, $numRows, $columns);
			return count($res);
		}
		 ...
		 */
		public function insert($tableName, $insertData, $id = false){
			$this->_query = "INSERT into $tableName";
			$stmt = $this->_buildQuery(null, $insertData);
			$stmt->execute();
			$this->reset();

			if ($id) return ($stmt->affected_rows > 0 ? $stmt->insert_id : false);
